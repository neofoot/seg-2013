...
public class Query {
	...
	/**
	 * Constructor method to create a new <code>Query</code> from a single
	 * keyword
	 * 
	 * @param w
	...
	public Query(WordTable wt, String w) {
		...
		if (newDocs != null) {
			// must clone newDocs so that changes to the doc counts of its
			// objects
	 ...
	 *          <code>NotPossibleException</code>, else modifies
	 *          <code>this</code> to be a query for <code>w</code> and all the
	 *          keywords already in <code>this</code>.
	 * @version 4.0
	 * @see "Program development in Java", pg 329
	 * 
	 */
	public void addKey(String w) throws NotPossibleException {
		...
		// for each current match, look up the document in the hash table and if
		// it
		...
		for (int i = 0; i < matches.size(); i++) {
			...
			boolean containsKeyword = false; // whether or not the current match
	 ...
	 * @modifies <code>this</code>
	 * @effects If <code>this</code> is not empty and <code>d</code> contains
	 *          all the keywords of <code>this</code> then adds <code>d</code>
	 *          and its keyword entries in <code>h</code> to
	 *          <code>matches</code> as a query result, else does nothing
	 * 
	 * @version 4.0
	 */
	...
	/**
	 * 
	 * @return Iterator of Query matches or null if no matches.
	 */
